...
public class DefaultYoutubeTrackDetails implements YoutubeTrackDetails {
    ...
    private AudioTrackInfo loadTrackInfo() {
        ...
        return buildTrackInfo(videoId, videoDetails.get("title").text(), videoDetails.get("author").text(),
                temporalInfo, ThumbnailTools.getYouTubeThumbnail(videoDetails, videoId));
    }
    ...
    private AudioTrackInfo loadLegacyTrackInfo() {
        ...
        return buildTrackInfo(videoId, args.get("title").text(), args.get("author").text(), temporalInfo,
                ThumbnailTools.getYouTubeThumbnail(args, videoId));
    }
    ...
    private AudioTrackInfo buildTrackInfo(String videoId, String title, String uploader, TemporalInfo temporalInfo,
        ...
        return new AudioTrackInfo(title, uploader, temporalInfo.durationMillis, videoId, temporalInfo.isActiveStream,
                WATCH_URL_PREFIX + videoId, thumbnail, null, false);
    }
    ...
    private static class TemporalInfo {
        ...
        public static TemporalInfo fromRawData(boolean wasLiveStream, JsonBrowser durationSecondsField,
            ...
            if (wasLiveStream && !legacy) {
                // Premiers have total duration info field but acting as usual stream so when we
                // try play it we don't know
                // current position of it since YT don't provide such info so assume duration is
                // unknown.
                isActiveStream = true;
                durationValue = 0;
            } else {
                // VODs are not really live streams, even though that field in JSON claims they
                // are. If it is actually live, then
