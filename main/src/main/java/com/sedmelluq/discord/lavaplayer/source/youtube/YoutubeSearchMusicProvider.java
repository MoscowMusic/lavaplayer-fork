...
public class YoutubeSearchMusicProvider implements YoutubeSearchMusicResultLoader {
    ...
    private AudioTrack extractMusicTrack(JsonBrowser jsonBrowser, Function<AudioTrackInfo, AudioTrack> trackFactory) {
        JsonBrowser thumbnail = jsonBrowser.get("musicResponsiveListItemRenderer").get("thumbnail")
        ...
        if (!lastElement.get("navigationEndpoint").isNull()) {
            // The duration element should not have this key, if it does, then duration is
            // probably missing, so return
            return null;
        }
        ...
        AudioTrackInfo info = new AudioTrackInfo(title, author, duration, videoId, false,
                WATCH_URL_PREFIX + videoId, ThumbnailTools.getYouTubeMusicThumbnail(thumbnail, videoId), null, false);
